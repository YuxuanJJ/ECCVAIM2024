...
import subprocess
import model_RTVSRx4



def parse_args():
    parser = argparse.ArgumentParser(description='VSR Testing')
    parser.add_argument('--net', type=str, default='./model_RTVSRx4.py')
    parser.add_argument('--checkpoint', type=str, default='./model_RTVSRx4.pth')
    parser.add_argument('--input', default='Sparks-Sunrise_3840x2160_60fps_8b_420_4x_crf39.mp4', help='input video')
    parser.add_argument('--output', default='./RTVSRx4', help='output path')
...
def main():
    ...
    for iFrame in range(0, num_frames, frames_per_batch):
        ...
        for j in range(current_batch_size):
            write_frame_yuv('./SRtempx4.yuv', out[j].clamp(0.0, 1.0).detach().cpu().numpy(), 8, mode)
            mode = 'a'
    ...
    ffmpeg_command = [
        'ffmpeg',
        '-f', 'rawvideo',
        '-video_size', f'{width*4}x{height*4}',
    ...
    if os.path.isfile('./SRtempx4.yuv'):
        os.remove('./SRtempx4.yuv')
