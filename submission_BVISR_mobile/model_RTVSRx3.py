...
class Generator(nn.Module):
    def __init__(self, upscale_factor=3) -> None:
        ...
        self.upsample2 = nn.Sequential(
            UpsampleOneStep(scale=3, num_feat=num_c, num_out_ch=1),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        YUV_n = F.interpolate(input, scale_factor=3, mode='nearest', recompute_scale_factor=True)
        x = self.upsample2(x) + YUV_n[:, 0:1, :, :]

        YUV = F.interpolate(input, scale_factor=3, mode='bicubic', recompute_scale_factor=True)
        UV = YUV[:, 1:3, :, :]
